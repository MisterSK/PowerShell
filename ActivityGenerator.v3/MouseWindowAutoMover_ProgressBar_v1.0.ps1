...
function Show-ProgressBar {
    ...
    $progressBar.Text = "$Activity in progress..."
    $form = [System.Windows.Forms.Form]::new()
    $form.Controls.Add($progressBar)
    $form.ShowDialog() | Out-Null
    
    # Update progress bar
    for ($i = 0; $i -le $Interval; $i++) {
        $progressBar.Value = $i
...
try {
    while ($true) {
        ...
        if (($now - $lastMouseMove).TotalMilliseconds -ge $mouseInterval) {
            Show-ProgressBar -Interval $mouseInterval -Activity "Moving Mouse"
            Move-MouseRandom
            $lastMouseMove = $now
            $mouseInterval = Get-RandomInterval -MinMinutes 5 -MaxMinutes 10
        ...
        if (($now - $lastWindowMove).TotalMilliseconds -ge $windowInterval) {
            Show-ProgressBar -Interval $windowInterval -Activity "Moving Notepad Window"
            Move-NotepadWindow
            $lastWindowMove = $now
            $windowInterval = Get-RandomInterval -MinMinutes 6 -MaxMinutes 12
